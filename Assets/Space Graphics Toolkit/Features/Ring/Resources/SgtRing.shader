...
#define unity_WorldToObject GetWorldToObjectMatrix()
	...
	void Ext_SurfaceFunction0 (inout Surface o, inout ShaderData d)
	{
		...
		float4 finalColor = main;
			...
			{
				float3 worldLightDir = normalize(d.worldSpacePosition - _LightPosition[i].xyz);
				float  theta         = dot(d.worldSpaceViewDir, worldLightDir);

				lighting += tex2D(_LightingTex, theta * 0.5f + 0.5f) * main * _LightColor[i];

				#if _SCATTERING
					scattering += MiePhase2(theta, _ScatteringMie) * _LightScatter[i];
				#endif
			}
