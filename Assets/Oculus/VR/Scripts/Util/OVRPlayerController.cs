...
public class OVRPlayerController : MonoBehaviour
{
	...
	/// </summary>
	[Tooltip("[Deprecated] When enabled, snap rotatio" +
        "0n will happen about the center of the " +
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
            ...
            bool moveUp = Input.GetKey(KeyCode.Q) || Input.GetKey(KeyCode.PageUp);
            bool moveDown = Input.GetKey(KeyCode.E) || Input.GetKey(KeyCode.PageDown);
			...
			if ((moveForward && moveLeft) || (moveForward && moveRight) ||
                ...
                || (moveUp && moveLeft) || (moveUp && moveRight) || (moveUp && moveForward) || (moveUp && moveBack)
                || (moveDown && moveLeft) || (moveDown && moveRight) || (moveDown && moveForward) || (moveDown && moveBack))
				MoveScale = 0.70710678f;

            // No positional movement if we are in the air
            if (!Controller.isGrounded && GravityModifier != 0)
				MoveScale = 0.0f;
            ...
            float rotInfluence = SimulationRate * Time.deltaTime * RotationAmount * RotationScaleMultiplier;
            Vector2 secAxis = OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick);
            if (secAxis.y < 0.0f)
                MoveThrottle += ort * (Mathf.Abs(secAxis.y) * transform.lossyScale.y * moveInfluence *
                                       BackAndSideDampen * Vector3.down);

            if (secAxis.y > 0.0f)
                MoveThrottle += ort * (secAxis.y * transform.lossyScale.y * moveInfluence * BackAndSideDampen *
                                       Vector3.up);
